	...
	public TelaProdutosFrame() {
		...
		btnNewButton_3.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String tipo=tipoMovimentacao=="ENTRADA" ? "Entrada":"Saida";
				if (tipoMovimentacao=="ENTRADA"){
					tipo = "Entrada";
					entradaProduto(produto, tipo);
				}
				else {
					tipo = "Saida";
					saidaProduto(produto,tipo);
				}
			...
			private void saidaProduto(Produto produto, String tipo) {
				tipo = "Saida";
				int quantidade = telaMovimentacao.getQuantidade();				
				try {
					Class<?> classe = Class.forName("br.com.allan."+tipo);
					Movimentacao movimentacao = (Movimentacao) classe.getDeclaredConstructor(null).newInstance();
					produto = movimentacao.movimentar(produto, quantidade);
					Dao dao = new Dao();
					dao.atualizar(produto);
					System.out.println(produto.toString());
					modelo.fireTableDataChanged();table.updateUI();					
				} catch (Exception e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				}
			}
			private void entradaProduto(Produto produto, String tipo) {
				int quantidade = telaMovimentacao.getQuantidade();
				double custo = telaMovimentacao.getCusto();				
				try {
					Class<?> classe = Class.forName("br.com.allan."+tipo);
					Movimentacao movimentacao = (Movimentacao) classe.getDeclaredConstructor(null).newInstance();
					if (telaMovimentacao.confirm==true) {
